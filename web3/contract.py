...
class ContractConstructor:
    ...
    def transact(self, transaction=None):
        ...
        res = self.web3.thk.sendTx(con_sign_tx)
        return res
    ...
    def call(self, transaction=None):
        ...
        if 'nonce' not in call_transaction:
            ...
            if isinstance(accountInfo, dict) and "nonce" in accountInfo:
                call_transaction.setdefault("nonce", str(accountInfo["nonce"]))
            else:
        ...
        if 'useLocal' not in call_transaction:
            call_transaction.setdefault('useLocal', False)
        if 'extra' not in call_transaction:
            call_transaction.setdefault('extra', "")
        if 'value' not in call_transaction:
...
def call_contract_function(
    ...
    call_transaction = web3.thk.signTransaction(call_transaction, web3.thk.defaultPrivateKey)
    return_data = web3.thk.callTransaction(call_transaction)
