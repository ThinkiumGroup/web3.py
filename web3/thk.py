...
class Thk(Module):
    ...
    def callTransaction(self, transaction):
        return self.web3.manager.request_blocking(
    ...
    def getCommittee(self, chainId, epoch):
        return self.web3.manager.request_blocking(
            "GetCommittee ",
            {
                "chainId": chainId, "epoch": epoch
            }
        )

    def RpcMakeVccProof(self, transaction):
        return self.web3.manager.request_blocking(
            "RpcMakeVccProof", transaction
        )

    def MakeCCCExistenceProof(self, transaction):
        return self.web3.manager.request_blocking(
            "MakeCCCExistenceProof", transaction
        )
